...
if (isset($_POST["signin"])) {
  ...
  if ($count == 1) {
    ...
    setcookie("HTH_USER_ID", $data["user_id"], time() + (86400 * 30), "/");
    setcookie("HTH_USER_ROLE", $data["user_role"], time() + (86400 * 30), "/");
    setcookie("HTH_USER_USERNAME", $data["user_name"], time() + (86400 * 30), "/");
...
if (isset($_POST["addPost"])) {
  ...
  $postTitle          = TestData($_POST["postTitle"]);
  $postDetails        = TestData($_POST["postDetails"]);
  ...
  $fileSize                   = $_FILES["postImage"]["size"] / 1024;
  ...
  if (
    ...
    $postImage = "catalog/posts/".$newFileName;

    //File upload path
    $uploadPath = "../catalog/posts/".$newFileName;
    ...
    if (move_uploaded_file($fileTmpName, $uploadPath)) {
      ...
      $query = mysqli_query($connection, "INSERT INTO `posts` (`post_id`, `category_id`, `post_title`, `post_details`, `post_image`, `post_date`, `post_views`, `post_comments`, `post_status`) 
      VALUES (NULL,'$categoryId','$postTitle', '$postDetails', '$postImage', '$postDate', '$postViews', '$postComments','save')") or die(mysqli_error($connection));
      ...
      if ($query) {
        ...
        $alert  = "success";
        $msg    = "You have successfully registered new post!";
        require("templates/alert.php");
      }
    } else {
      $alert  = "danger";
...
if (isset($_POST["updatePost"])) {
  ...
  if (
    ...
    if (move_uploaded_file($fileTmpName, $uploadPath)) {
      ...
      $query = mysqli_query($connection, "UPDATE `posts` SET `category_id` = '$categoryId', `post_title` = '$postTitle', `post_details` = '$postDetails', `post_image` = '$postImage' WHERE `posts`.`post_id` = '$postId'") or die(mysqli_error($connection));
...
if (isset($_GET["deletePost"])) {
  ...
  $query = mysqli_query($connection, "UPDATE posts SET post_status ='deleted' WHERE `post_id` = '$postId'") or die(mysqli_error($connection));
...
if (isset($_GET["publishPost"])) {
  ...
  $postId      = TestData($_GET["publishPost"]);
  $query = mysqli_query($connection, "UPDATE posts SET post_status ='published' WHERE `post_id` = '$postId'") or die(mysqli_error($connection));
  ...
  if ($query) {
    ...
    $alert  = "success";
    $msg    = "You have successfully published new post!";
    require("templates/alert.php");
  }
}
